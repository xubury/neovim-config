...
class EmmyDebugSession extends DebugSession_1.DebugSession {
    ...
    if (!args.ideConnectDebugger) {
      ...
      const socket = net
        .createServer((client) => {
          ...
          this.sendEvent(new vscode_debugadapter_1.Event("onNewConnection"));
        })
        .listen(args.port, args.host)
        .on("listening", () => {
          this.sendEvent(
            new vscode_debugadapter_1.OutputEvent("hello world", "stderr")
          );
          this.sendEvent(
        ...
        .on("error", (err) => {
          this.sendEvent(
            new vscode_debugadapter_1.OutputEvent(`${err}`, "stderr")
          );
  ...
  onSocketClose() {
    ...
    this.sendEvent(new vscode_debugadapter_1.OutputEvent("Disconnected.\n"));
    if (this.listenMode) {
  ...
  evaluateRequest(response, args) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (evalResp.success) {
        const emmyVar = new EmmyDebugData_1.EmmyVariable(evalResp.value);
